...
# 	BidTheatre.hype-export.py
#		Export Script for Tumult Hype to produce HTML5 ads for BidTheatre:
#			https://bidtheatre.zendesk.com/hc/en-us/articles/210319565-HTML5-Creatives
#
#		Installation, usage, and additional info: 
#			https://tumult.com/hype/export-scripts/
#
#		MIT License
#		Copyright (c) 2021 Tumult Inc.
#
...
version_info_url = "https://static.tumult.com/hype/export-scripts/BidTheatre/latest_script_version.txt" # only returns a version number
download_url = "https://tumult.com/hype/export-scripts/BidTheatre/" # gives a user info to download and install
minimum_update_check_duration_in_seconds = 60 * 60 * 24 # once a day
defaults_bundle_identifier = "com.tumult.Hype2.hype-export.BidTheatre"
...
def main():
	...
	##				arguments: array of strings
		...
		metadata_path = os.path.join(args.modify_staging_path, "metadata.json".encode("utf-8"))
		template = string.Template(metadata_json)
		metadata_json_contents = template.substitute({'width' : export_info['main_container_width'], 'height' : export_info['main_container_height'], "clickTag" : click_tag })
		with open(metadata_path, 'w') as target_file:
			target_file.write(metadata_json_contents)
...
if __name__ == "__main__":
	main()
||||||| parent of a8f982c (BidTheatre Export Script)
=======
#!/usr/bin/python

# 	BidTheatre.hype-export.py
#		Export Script for Tumult Hype to produce standard HTML5 ads for BidTheatre.
#		Following instructions from:
#			https://help.bidtheatre.com/en/articles/4701905-html5-creatives
#
#		Installation, usage, and additional info: 
#			https://tumult.com/hype/export-scripts/
#
#		MIT License
#		Copyright (c) 2025 Tumult Inc.
#
...
insert_at_body_end = """
	<!-- Start BidTheatre clickurl exit -->
	<script>
		function adExit() {
			console.log("clickTag = ${clickTag}");
			var clickurl = asxhtml5.getParam('clickurl', '${clickTag}');
			console.log("clickurl" + clickurl);
			window.open(clickurl, '_blank');
		}
	</script>
	<!-- End BidTheatre clickurl exit -->
	"""
...
def main():
	...
	##				arguments: array of strings
		...
		# modify BidTheatre metadata File
		global metadata_file_content
		template = string.Template(metadata_file_content)
		
		# check if clickTAG was defined in Export Script Arguments
		if "clickTag" in export_info["document_arguments"]:
			click_tag = export_info["document_arguments"]["clickTag"]
		else:
			click_tag = ""
		...
		metadata_file_content = template.substitute({'name' : title, 'width' : export_info['main_container_width'], 'height' : export_info['main_container_height'] })
		
		# write out BidTheatre metadata File
		write_metadata(args.modify_staging_path)

		# insert clickTag into head end
		global insert_at_body_end
		template = string.Template(insert_at_body_end)
		insert_at_body_end = template.substitute({"clickTag" : click_tag })
...
def write_metadata(folder_path):
...
if __name__ == "__main__":
	main()
>>>>>>> a8f982c (BidTheatre Export Script)
