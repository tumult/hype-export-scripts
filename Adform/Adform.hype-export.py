...
#		Export Script for Tumult Hype to produce standard HTML5 ads for Adform Studio.
#		Originally written by Raphael Walser (Raphii)
#
#		Installation, usage, and additional info: 
#			https://tumult.com/hype/export-scripts/
#
#		MIT License
#		Copyright (c) 2024 Tumult Inc.
#		Copyright (c) 2020 raphii.ch
#
...
def main():
	...
	if args.get_options:		
		...
		def document_arguments():
			return ["Version", "Description", "clickTag", "Event"];
		
		def extra_actions():
			return [
				{"label" : "Exit", "function" : "adformExit"},
			]
	...
	##				arguments: array of strings
		...
		# modify Adform manifest File
		global manifest_file_content
		template = string.Template(manifest_file_content)
		...
		# check if Event was defined in Export Script Arguments
		if "Event" in export_info["document_arguments"]:
			event = export_info["document_arguments"]["Event"]
			if not event:
				event = "banner click"
		else:
			event = "banner click"
		...
		# get Version number or set to 1.0
		if "Version" in export_info["document_arguments"]:
			version = export_info["document_arguments"]["Version"]
			if not event:
				version = "1.0"
		else:
			version = "1.0"
		...
		# get Description
		if "Description" in export_info["document_arguments"]:
			description = export_info["document_arguments"]["Description"]
		else:
			description = ""
		
		manifest_file_content = template.substitute({'version' : version, 'title' : title, 'description' : description, 'width' : export_info['main_container_width'], 'height' : export_info['main_container_height'], "event" : event, "clickTag" : click_tag })
		
		# write out Adform manifest File
		write_manifest(args.modify_staging_path)

		# insert clickTag into head end
		global insert_at_body_end
		template = string.Template(insert_at_body_end)
		insert_at_body_end = template.substitute({"clickTag" : click_tag, "event" : event })
...
def write_manifest(folder_path):
