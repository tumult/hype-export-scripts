...
# 	DeltaProjects.hype-export.py
#		Basic creation of a Delta Projects ad from a Tumult Hype document
#
#		Based on documentation from:
#			https://docs.deltaprojects.com/html5/
#
...
version_info_url = "https://static.tumult.com/hype/export-scripts/DeltaProjects/latest_script_version.txt" # only returns a version number
download_url = "https://tumult.com/hype/export-scripts/DeltaProjects/" # gives a user info to download and install
minimum_update_check_duration_in_seconds = 60 * 60 * 24 # once a day
defaults_bundle_identifier = "com.tumult.Hype2.hype-export.DeltaProjects"
...
def main():
	...
	##				arguments: array of strings
		...
		template = string.Template(manifest_json)
		manifest_json = template.substitute({"title" : title, 'width' : export_info['main_container_width'], 'height' : export_info['main_container_height'], "clickTag" : click_tag })
		
		manifest_json_path = os.path.join(args.modify_staging_path, "manifest.json")
		
		with open(manifest_json_path, 'w') as target_file:
			target_file.write(manifest_json)
