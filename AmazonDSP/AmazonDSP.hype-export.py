...
defaults_bundle_identifier = "com.tumult.Hype2.hype-export.AmazonDSP"
...
def main():
	...
	##				arguments: array of strings
		...
		inline_hype_scripts(index_path)
...
def inline_hype_scripts(filename):
	import re
	...
	# Only match <script> tags whose src ends in a HYPE-*.js filename
	pattern = re.compile(
		r'(<script\b[^>]*?)\s+src="([^"]*HYPE-[^"/]+\.js)"([^>]*>)\s*</script>',
		re.IGNORECASE | re.DOTALL
	)
	
	def replacer(match):
		opening, src_path, end = match.group(1), match.group(2), match.group(3)
		js_file = os.path.normpath(os.path.join(base_dir, src_path))
		...
		# Delete the JS file now that we've read it
		try:
			os.remove(js_file)
		except OSError as e:
			sys.stderr.write("Warning: could not delete %s: %s\n" % (js_file, e))
		
		# Inject the JS inline, preserving type/charset attrs
		return opening + end + "\n" + js_content + "\n</script>"
